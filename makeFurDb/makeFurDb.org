...
sequences. To ensure there are no blanks in a sequence header that is
later passed to Blast, we truncate it after the first token, usually
the accession. Then we store the sequence.
#+end_export
#+begin_src go <<Read target representative, Pr. \ref{pr:mfd}>>=
  ...
  for sc.ScanSequence() {
	  seq := sc.Sequence()
	  //<<Truncate header, Pr. \ref{pr:mfd}>>
	  //<<Store sequence, Pr. \ref{pr:mfd}>>
  }
#+end_src
#+begin_export latex
We truncate the header at the first blank and recreate the sequence
with the new header.
#+end_export
#+begin_src go <<Truncate header, Pr. \ref{pr:mfd}>>=
...
reverse strands of the sequences it contains, and close the file
again.
#+end_export
...
#+end_src
#+begin_export latex
When doing exact matching, we ignore masking, so we convert the
nucleotides of the forward and reverse strands we just read to upper
case.
#+end_export
#+begin_src go <<Read sequences of target rep, Pr. \ref{pr:mfd}>>=
...
slices of length $\ell=n/T$. The last of these slices might extend
beyond the length of the slice of neighbor names. We make sure that
doesn't occur.
#+end_export
...
It turns out that calculating the ESA only from the forward strand
saves a bit of time and a lot of and memory. For detailed measurements
on this, please take a look at the program \ty{stream}, which is also
part of this package. Before we calculate the ESA from the forward
strand, we convert it to upper case.
#+end_export
...
We come to the penultimate element of the \ty{fur} database, the Blast
database of the neighbors. To construct this, we iterate over the
neighbor files and convert each one to a Blast database. These
databases are numbered, hence we also declare a counter, $c$. To
facilitate subsequent handling of the databases, we link them into one
entity.
#+end_export
#+begin_src go <<Write Blast database of neighbors to \ty{n}, Pr. \ref{pr:mfd}>>=
  ...
  for neighbor, _ := range neighbors {
	  //<<Make Blast database, Pr. \ref{pr:mfd}>>
  }
  //<<Link Blast databases, Pr. \ref{pr:mfd}>>
#+end_src
#+begin_export latex
We need to preserve any masking information contained in the neighbor
sequences. To do this, we first extract the mask, then apply it when
constructing the database with \ty{makeblastdb}.
#+end_export
#+begin_src go <<Make Blast database, Pr. \ref{pr:mfd}>>=
  //<<Extract mask, Pr. \ref{pr:mfd}>>
  //<<Run \ty{makeblastdb}, Pr. \ref{pr:mfd}>>
#+end_src
#+begin_export latex
We extract the mask by running the program \ty{convert2blastmask},
which is part of the Blast package.
#+end_export
#+begin_src go <<Extract mask, Pr. \ref{pr:mfd}>>=
  p := *optN + "/" + neighbor
  o := *optD + "/mask.asnb"
...
#+end_src
#+begin_export latex
We run \ty{makeblastdb}, which is also part of the Blast package, and
include the masking information we just created in the new
database. To ensure each database has a unique name, we call them
\ty{n}$c$, where $c$ is iterated in each round.
#+end_export
#+begin_src go <<Run \ty{makeblastdb}, Pr. \ref{pr:mfd}>>=
  p = *optN + "/" + neighbor
  c++
...
#+end_src
#+begin_export latex
To link the databases, we first write the names of the databases to
the file \ty{dblist.txt}. The actual linking is then done by calling
another program from the Blast package, \ty{blastdb\_aliastool}.
#+end_export
#+begin_src go <<Link Blast databases, Pr. \ref{pr:mfd}>>=
...
The last element of the fur database is a file, \ty{n.txt}, containing
the number of nucleotides in the database and its GC content. This is
later used by \ty{fur} to calculate the null distribution of match
lengths. So we create \ty{n.txt}, get the entries from the Blast
database of neighbors we just created, scan these entries, and finally
print the statistics we are after.
#+end_export
#+begin_src go <<Write statistics of neighbors to \ty{n.txt}, Pr. \ref{pr:mfd}>>=
  //<<Create \ty{n.txt}, Pr. \ref{pr:mfd}>>
  //<<Get entries from neighbors Blast database, Pr. \ref{pr:mfd}>>
  //<<Scan entries from neighbors Blast database, Pr. \ref{pr:mfd}>>
  //<<Print neighbors statistics, Pr. \ref{pr:mfd}>>
#+end_src
#+begin_export latex
We create \ty{n.txt} and make sure it is eventually closed again.
#+end_export
#+begin_src go <<Create \ty{n.txt}, Pr. \ref{pr:mfd}>>=
  w, err := os.Create(*optD + "/n.txt")
  util.Check(err)
  defer w.Close()
#+end_src
#+begin_export latex
We get the entries from the neighbors Blast database by calling the
program \ty{blastdbcmd} on all database entries.
#+end_export
#+begin_src go <<Get entries from neighbors Blast database, Pr. \ref{pr:mfd}>>=
...
#+end_src
#+begin_export latex
We iterate over the neighbor sequences, one FASTA entry at a time and
count the nucleotides, \ty{[ACGTacgt]}. We think of their count as the
sequence length, $\ell$. We also count the GCs, \ty{[GCgc]}, $g$.
#+end_export
#+begin_src go <<Scan entries from neighbors Blast database, Pr. \ref{pr:mfd}>>=
  ...
  for sc.ScanSequence() {
	  d := sc.Sequence().Data()
	  for i, _ := range d {
		  //<<Count nucleotides, Pr. \ref{pr:mfd}>>
		  //<<Count GCs, Pr. \ref{pr:mfd}>>
	  }
  }
#+end_src
#+begin_export latex
We count the upper and lower case versions of the standard four
nucleotides. If we are not dealing with a nucleotide, we skip the
subsequent test for \ty{G} or \ty{C}.
#+end_export
#+begin_src go <<Count nucleotides, Pr. \ref{pr:mfd}>>=
...
#+end_src
#+begin_export latex
We count the nucleotides that are either \ty{G} or \ty{C}.
#+end_export
#+begin_src go <<Count GCs, Pr. \ref{pr:mfd}>>=
  if bytes.ContainsAny(d[i:i+1], gc) {
	  g++
  }
#+end_src
#+begin_export latex
We convert $g$ to the GC content, $g_{\rm c}$, and print it together
with the sequence length.
#+end_export
#+begin_src go <<Print neighbors statistics, Pr. \ref{pr:mfd}>>=
