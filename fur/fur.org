...
#+end_src
#+begin_export latex
Step~(\ref{eq:fur2}) is delegated to the \ty{Intersection} function,
for which we set the sensitivity threshold. The user also can restrict
the output from \ty{Intersect} to exact matches. She can again also
print the result of this step and exit.
#+end_export
...
also set the minimum length of the final regions at this point.
#+end_export
...
header and extends it with the field \ty{\$x}:
\begin{center}
\ty{name\$x},
\end{center}
where \ty{x} is a zero-based start coordinate of the region on the
representative contig. This field will be parsed in
Step~\ref{eq:fur2}.

The second format extends the \ty{name} field with a suffix preceded by
an underscore:
\begin{center}
\ty{name\_(start..end)},
\end{center}
where start and end are one-based, end exclusive. We use this format
...
function. This takes as input a struct of parameters. Like at the end
...
#+begin_src go <<Run step (\ref{eq:fur2}), Pr. \ref{pr:fur}>>=
  ...
  if numTargets > 1 {
	  ...
	  //<<Process \ty{Intersect} output, Pr. \ref{pr:fur}>>
  }
...
#+end_src
#+begin_export latex
We initialize a struct of parameters for \ty{Intersect}:
\begin{itemize}
  \itemsep0em
  \item \ty{Reference}: a slice containing the unique regions we just
    extracted. They serve as the reference, onto which \ty{Intersect}
    piles the remaining target sequences;
    \item{\ty{ShiftRefRight}: use the \ty{\textbackslash}\ty{x} field
    ...
    \item \ty{Threshold}: sensitivity of intersection, the minimum
      fraction of genomes in \ty{TargetDir} that have to share the
      same nucleotide at a given position in the \ty{Reference}; we
      have to interpret the value of the \ty{-f} first;
    \item \ty{ShustrPval}: a p-value of the shustring length to
      calculate the minimum anchor length for exact matching;
    ...
    \item \ty{PrintN}: report segregation sites' nucleotides as \ty{N}
      in the output \ty{fasta} sequences. This parameter corresponds
      to the negated value of the \ty{-x} flag, that is, if the user
      opted for exact matches only, we don't print \ty{N}s;
    \item \ty{PrintSegSitePos}: add information on segregation sites
...
#+end_src
#+begin_export latex
The sensitivity threshold $f$ can take values such as $0 < f \le
1$. We check the \ty{optF} value and use it as the threshold if it is
...
#+end_src
#+begin_export latex
We remove short regions from the intersection.  After this, we process
data and headers of the remaining regions.
#+end_export
#+begin_src go <<Process \ty{Intersect} output, Pr. \ref{pr:fur}>>=
  //<<Remove short regions, Pr. \ref{pr:fur}>>
  //<<Process data and headers, Pr. \ref{pr:fur}>>
#+end_src
#+begin_export latex
...
#+end_src
#+begin_export latex
The header of the target representative and the interval coordinates
are joined into one string by an underscore to generate unique headers
for the subsequent Blast step.
#+end_export
#+begin_src go <<Process data and headers, Pr. \ref{pr:fur}>>=
  for _, region := range regions {
	  //<<Insert underscores into headers, Pr. \ref{pr:fur}>>
  }
#+end_src
#+begin_export latex
We insert an underscore instead of a tab right after the chromosome
(or contig) name and before an open parenthesis. We expect the
parenthesis to occur only once in a header as chromosome names
\textit{usually} do not contain parentheses.
#+end_export
#+begin_src go <<Insert underscores into headers, Pr. \ref{pr:fur}>>=
...
#+end_src
#+begin_export latex
We construct a set of tests, read the expected outputs,create a struct
of test cases and iterate over them.
#+end_export
#+begin_src go <<Testing, Pr. \ref{pr:fur}>>=
  //<<Construct tests, Pr. \ref{pr:fur}>>
  //<<Read the expected outputs, Pr. \ref{pr:fur}>>
  tests := []struct {
	  name string
	  cmd *exec.Cmd
	  want []byte
  }{
	  //<<Fill the tests struct, Pr. \ref{pr:fur}>>
  }
...
#+end_src
#+begin_export latex
We read the expected results of successful tests from files and save
them in a slice. The results are contained in files \ty{r1.txt},
\ty{r2.txt}, and so on.
#+end_export
#+begin_src go <<Read the expected outputs, Pr. \ref{pr:fur}>>=
  var wants [][]byte
  for i := 0; i < 7; i++ {
...
#+end_src
#+begin_export latex
We construct three tests with repeat masking. In the first, we request
...
#+end_src
#+begin_export latex
We populate the struct of \ty{tests}.
#+end_export
#+begin_src go <<Fill the tests struct, Pr. \ref{pr:fur}>>=
  // Without masking
