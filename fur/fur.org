...
#+end_src
#+begin_export latex
We remove the exclamation marks inserted by \ty{Intersect} in the
sequences' data. They are used as separators between concatenated
contigs. Sequences that are, in fact, contiguous, could get splitted
into separate contigs during genome assembly. \ty{Intersect} may
occasionally find such matches.
...
#+end_src
#+begin_export latex
We insert an underscore instead of a whitespace right after the
chromosome (or contig) name and before an open parenthesis. We expect
the parenthesis to occur only once in a header as chromosome names
